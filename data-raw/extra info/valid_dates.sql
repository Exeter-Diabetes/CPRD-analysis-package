...
create table cprd_data.r_valid_date_lookup as select c.patid, dob,
...
(select patid, dob, least(if(cprd_ddate is null,str_to_date('1/1/2050','%d/%m/%Y'),cprd_ddate), 
...
(select a.patid, if(a.mob is NULL, str_to_date(concat('1/7/',a.yob),'%d/%m/%Y'), str_to_date(concat('15/',a.mob,'/',a.yob),'%d/%m/%Y')) as dob, a.cprd_ddate, a.regenddate, b.lcd from cprd_data.patient a left join cprd_data.practice b on a.pracid=b.pracid)
as T1)
