...
select * from cprd_data.r_valid_date_lookup where year(gp_ons_end_date)=2050 or year(gp_ons_hes_all_end_date)=2050 or year(gp_ons_hes_nph_filtered_end_date)=2050;
# None

select max(gp_ons_end_date), max(gp_ons_hes_all_end_date), max(gp_ons_hes_all_end_date) from cprd_data.r_valid_date_lookup;
# All are 15/10/2020
